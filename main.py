...
def generate_content(news_text):
    ...
    **1. 헤더 (Header)**
    - `📦 오늘의 반도체 뉴스` (제목, 볼드체, 크게)
    - 아랫줄로 옮겨서, ` {today_date} | 발행인: 반도체재료개발TFT 김동휘` (날짜 및 발행인, 작게)  
    - 한 줄 띄웁니다.
    ...
    **4. Technical Term (용어 해설)**
    - `📚 Technical Term`
    - 아랫줄로 옮겨서, [용어명 (한글/영어)]
    - 아랫줄로 옮겨서, Technical Term: 'BSPDN', 'Glass Substrate', 'Hybrid Bonding' 등 반도체 전문가 수준의 심도 있는 기술 용어 1개를 선정해 상세히 설명하세요.
